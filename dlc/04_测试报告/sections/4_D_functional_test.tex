...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        M7 导出下载功能 & 10 & 9 & 1 & 90\% \\
        \hline
        \textbf{合计} & \textbf{56} & \textbf{54} & \textbf{2} & \textbf{96.4\%} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        Google Chrome & 120+ & 正常 & 正常 \\
        \hline
        Microsoft Edge & 120+ & 正常 & 正常 \\
        \hline
        Mozilla Firefox & 120+ & 正常 & 富文本光标偶有偏移 \\
        \hline
        Apple Safari & 17+ & 正常 & CSS Grid间距轻微偏差 \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|}
        ...
        \hline
        后端环境 & SpringBoot 3.x, PostgreSQL 17, Redis 7 \\
        \hline
...
学生端7大功能模块共56条测试用例，执行通过54条，通过率96.4\%。2条失败用例均为导出模块：DOCX导出因LibreOffice依赖在未安装环境下不可用（BUG-001），PDF导出在大文件场景下超时。核心业务流程（注册→登录→创建论文→编辑章节→添加参考文献→导出）完整可走通，功能覆盖需求规格说明书中定义的22条功能需求。综合评定：功能完整，可交付。

\subsection{界面质量评价}

学生端界面整体视觉一致，基于Element Plus设计系统统一组件风格。编辑器三栏布局交互流畅，面板拖拽调整宽度、章节拖拽排序等高级交互功能实现良好。响应式适配在1920×1080到375×667范围内均正常渲染。浏览器兼容性方面Chrome/Edge完全通过，Firefox/Safari有轻微样式差异不影响功能使用。综合评分：优秀。

\subsection{改进建议}

\begin{enumerate}
    \item 导出模块建议用Apache PDFBox + FlyingSaucer替代LibreOffice，减小Docker镜像约200MB
    \item 编辑器图片上传后建议增加进度条反馈，大图上传时用户体验更好
    \item 论文列表搜索功能建议增加学院/状态筛选项，提升检索效率
    \item 移动端（375px）下编辑器的工具栏可考虑折叠为悬浮菜单，节省屏幕空间
\end{enumerate}
